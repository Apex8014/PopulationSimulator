...
if askForYN("Do you want to use a preset simulation?"):
    preset = input("Choose a preset(1 (First test), 2 (Base), 3 (Previous highscore: 1150 generations), 4 (Previous highscre: 1380 generations), 5 (previously most consistent), 6 (previous most consistent and previous highscore: 1669 generations), 7 (current most consistent & current highscore: 2400 generations)), 8 (AI current highscore: 66900+ generations): ")
    while preset not in ["1","2","3","4","5","6","7","8"]:
        preset = input("Please enter 1, 2, 3, 4, 5, 6, 7, or 8: ")
    if preset == "1":
        ...
        plant = Organism("Berries", 988599906, 0.0417, 955764494)
        herbivore = Organism("Arctic hares", 199875696, 1.5965, 904334644, 0.0, [plant])
        omnivore = Organism("Arctic Foxes", 512099987, 1.9334, 402372544, 1.2317, [herbivore,plant])
        predator = Organism("Perigine Falcons", 328045798, 0.9898, 669684780, 1.6439, [herbivore,omnivore])
else:
